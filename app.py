...
pipe_sgd = joblib.load(open('models/books_clf_sgd_30_04_2022.pkl','rb')) 
...
def predict_category_of_book(docx):
    results = pipe_sgd.predict([docx])
    return results[0]

def get_prediction_conf_score(docx):
    #results = pipe_lr.predict_proba([docx]) #In case we use logistic regression or random forest
    results = pipe_sgd.decision_function([docx])
    return results 
...
* **Python libraries:** BeautifulSoup, numpy, pandas, matplotlib, seaborn, neattext, sklearn, joblib, streamlit. 
...
""")
      ...
      if submit_text : 
            ...
            conf_score = get_prediction_conf_score(raw_text)
            ...
            with col1:
                ...
                st.write(prediction)
                st.write('Confidence score: {}'.format(np.max(conf_score)))

            with col2:
                st.success('Confidence scores for all categories')
                #st.write(probability)
                conf_df = pd.DataFrame(conf_score,columns=pipe_sgd.classes_)
                st.write(conf_df.T)
                conf_df_clean = conf_df.T.reset_index()
                conf_df_clean.columns = ['Categories','Confidence scores']

                fig = alt.Chart(conf_df_clean).mark_bar().encode(x='Categories',y='Confidence scores',color='Categories')
